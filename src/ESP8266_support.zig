...
pub fn NetworkDevice(binds: usize) type {
    return struct {
        ...
        pub fn set_network_mode(self: *Self, mode: DriverMode, recv_mode: Network.RecvMode) !void {
            const runner_inst = self.runner_loop.runner_instance;
            const free = self.runner_loop.get_tx_free_space(runner_inst);
            if (free < 2) return DriverError.TX_BUFFER_FULL;
            ...
            var cmd_slice = std.fmt.bufPrint(&pkg.str, "{s}{s}={d}{s}", .{
                prefix,
                "CIPRECVMODE",
                @intFromEnum(recv_mode),
                postfix,
            }) catch unreachable;
            ...
            cmd_slice = std.fmt.bufPrint(&pkg.str, "{s}{s}={d}{s}", .{
                prefix,
                get_cmd_string(.NETWORK_SERVER_CONF),
                self.div_binds,
                postfix,
            }) catch unreachable;
            pkg.len = cmd_slice.len;
        ...
        pub fn connect(self: *Self, id: usize, config: ConnectConfig) DriverError!void {
            ...
            if (config.timeout) |_| return DriverError.UNSUPPORTED;
        ...
        pub fn create_server(self: *Self, config: ServerConfig) DriverError!void {
            ...
            if (self.runner_loop.get_tx_free_space(runner_inst) < 2) return DriverError.TX_BUFFER_FULL;
